...
'Database type help'		=>	'На текущий момент Flazy поддерживает MySQL, PostgreSQL и SQLite. Если ваш тип БД отсутствует в выпадающем меню, это означает, что PHP не поддерживает особенности вашей БД.',
...
'Mysql InnoDB info'			=>	'Flazy определил, что ваш MySQL сервер, возможно, поддерживает <a href="http://dev.mysql.com/doc/refman/5.0/en/innodb.html">InnoDB</a>. Это было бы хорошим выбором, если вы планируете создание большого форума. Если вы не уверенны, не рекомендуется использовать InnoDB.',
'Database server help'		=>	'Введите адрес сервера БД (например: <em>localhost</em>, <em>mysql1.example.ru</em> или <em>208.77.188.166</em>). Вы можете назначить свой номер порта, если БД недоступна по стандартному (например: <em>localhost:3580</em>). Для SQLite введите что угодно или оставьте <em>localhost</em>.',
'Database name help'		=>	'Введите имя Базы Данных, в связке с которой будет установлен Flazy. Эта БД уже должна существовать. Для SQLite это относительный путь до файла БД. Если файл БД SQLite не существует, Flazy попытается его создать.',
...
'Table prefix help'			=>	'Введите префикс для таблиц БД. Определив префикс вы сможете использовать несколько копий Flazy с одной БД (например: <em>flazy_</em>).',
...
'Password help'				=>	'Минимум 6 символов. Чувствительно к регистру.',
...
'Board title info'			=>	'Введите название и короткое описание вашего форума Flazy. Они будут отображаться вверху каждой страницы. Оставьте поля пустыми, чтобы использовать заголовок и описание по умолчанию. Их можно будет изменить позже.',
'Base URL help'				=>	'Бызовый URL — это URL (без закрывающего слеша) вашего форума Flazy (например: <em>http://forum.example.ru</em> или <em>http://example.ru/~myuser</em>). Имейте ввиду, изначально прописанное в этом поле значение — просто догадка Flazy.',
...
'Missing prefix'			=>	'Вы должны ввести префикс БД. Пожалуйста, вернитесь и исправьте ошибку.',
